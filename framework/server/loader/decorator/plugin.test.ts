import { decoratorPlugin } from "./plugin"
import { tester } from "./tester"

const withArrobaImport: TestCase = {
...
const multipleFunctionDecoratorCase: TestCase = {
...
const parenthelessMultipleFunctionDecoratorCase: TestCase = {
...
const defaultFunctionDecoratorWithComplexArgsCase: TestCase = {
...
const innerBlockFunctionDecoratorMustBeThrowCase: TestCase = {
   sampling: '@outer(0) function Hello(props) { @inner(1) function what() { }; return 1 }',
   expected: 'Invalid nesting functions decorators',
   hasError: true
}

const innerArrowFunctionDecoratorMustBeThrowCase: TestCase = {
...
tester(true, false, scenarios, 'COMMENTS', code => 
   decoratorPlugin({code, path:'file:///fake.tsx'})?.contents)